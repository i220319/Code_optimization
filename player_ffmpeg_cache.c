...
int64_t avpkt_cache_update_discontinue_current_ms(play_para_t *player);
...
static int avpkt_cache_update_player_param(av_packet_cache_t *cache_ptr)
{
	...
	cache_ptr->audio_max_packet = am_getconfig_int_def("libplayer.cache.amaxframes", 5000);
	cache_ptr->video_max_packet = am_getconfig_int_def("libplayer.cache.vmaxframes", 2000);
	...
	if(cache_ptr->local_play)
		...
		cache_ptr->max_cache_mem = am_getconfig_int_def("libplayer.cache.maxmem", 20*1024*1024);
...
static void packet_queue_discard_frame(av_packet_cache_t *cache_ptr, PacketQueue *q)
{
    ...
    while (pkt && pkt->frame_id <= cache_ptr->discard_frame_id && q->cur_pkt->frame_id > cache_ptr->discard_frame_id) {
...
static int avpkt_cache_queue_valid_frame_id(PacketQueue *q,int64_t try_frame_id)
{
    ...
    if (q->frames_backward_level > 0.0) {
        max_seekbakcword = MAX(q->frames_max_seekbackword, RESERVED_PKT);
        if ((q->frames_for_seek_backward -try_cnt) > max_seekbakcword) {
            ret = 1;
        }
    } else {
...
static int avpkt_cache_queue_check_free_mem(av_packet_cache_t *cache_ptr,PacketQueue *q, int64_t try_frame_id)
{
	...
	while (pkt && pkt->frame_id <= try_frame_id) {
		...
		if (avpkt_cache_queue_used(cache_ptr, q)) {
			if (q->frames_backward_level > 0.0) {
				max_seekbakcword = MAX(q->frames_max_seekbackword, RESERVED_PKT);
				if ((q->frames_for_seek_backward - (try_cnt+1)) <= max_seekbakcword) {
					break;
				}
			} else {
				if (q->frames_for_seek_backward - (try_cnt+1) <= 0)
					break;
			}
		}
		try_cnt++;
	...
	if (seek_sucess == 1) {
		...
		*/
		if (q->seek_bykeyframe == 1) {
			...
			for (; mypktl_left != NULL; mypktl_left = mypktl_left->priv) {
				if (mypktl_left->pkt.flags & AV_PKT_FLAG_KEY)
					break;
				left++;
			}

			if (!(mypktl_left != NULL && left <= SEEK_KEY_FRAME_MAX_NUM)) {
				mypktl_right = mypktl_2;
				for (; mypktl_right != NULL; mypktl_right = mypktl_right->next) {
			...
			if (findkey_ok == 1) {
				...
				seek_sucess = 0;
				cache_unlock(&q->lock);
				log_print("[%s] cannot find keyframe!\n", __FUNCTION__);
				return -1;
			}
		}
...
void *avpkt_cache_reset_thread(void *arg)
{
    ...
    if (cache_ptr->backup_pkt.data != NULL) {
        av_free_packet(&cache_ptr->backup_pkt);
    }
    avpkt_cache_update_player_param(cache_ptr);
    if (cache_ptr->has_video == 1 && cache_ptr->video_index != -1) {
...
static int avpkt_cache_release(av_packet_cache_t *cache_ptr)
{
	...
	s_pkt_used_error = 0;
	#endif
    log_print("avpkt_cache_release ok\n");
    return 0;
}
...
int avpkt_cache_checkvlevel(av_packet_cache_t *cache_ptr, float level) {
	...
	if (player &&  player->vstream_info.has_video && player->state.video_bufferlevel >= level) {
		return 1;
	}
...
static int avpkt_cache_update_bufed_time(void) {
	...
	if (bak_calc_pts > 0) {
		cache_ms_bypts = (int64_t)((bak_calc_pts * s_avpkt_cache->queue_video->timebase) / 90);
		total_cache_ms = cache_ms_bypts;
	}
	bufed_time = (int)(total_cache_ms/1000);

	//log_print("bufed_time = %d s_avpkt_cache->starttime_ms = %lld",bufed_time,s_avpkt_cache->starttime_ms);
	bufed_time += (int)(s_avpkt_cache->starttime_ms/1000 );
...
static int64_t packet_calc_cachetime_by_player_current_ms(PacketQueue *q, int64_t player_current_ms, int stream_idx, int debug)
{
...

	{
		...
		if (player_current_ms > 0) {
			...
			log_debug("playtime_ms = 0x%lld, player_current_ms = 0x%lld\n", playtime_ms, player_current_ms);
			log_debug("starttime_ms = 0x%lld, seek_discontinue_current_ms = 0x%lld, discontinue_current_ms = 0x%lld\n", 
		...
		if (q->frame_dur_pts > 0) {
			...
			if (playtime_ms) {
				...
				log_debug("cache_ms: %lld, q->frames_in: %lld, q->frames_out: %lld, q->timebase = %f, playtime_ms = %lld, total_cache_ms = %lld\n",
							cache_ms, q->frames_in, q->frames_out, q->timebase, playtime_ms, total_cache_ms);
				...
				if (debug) {
					log_debug("bak_calc_pts:0x%llx, 0x%llx", bak_calc_pts, discontinue_pts);
					log_debug("cache_ms_by_duration:%lld, cache_ms_by_pts:%lld", cache_ms, cache_ms_bypts);
				}
			}
			vbuflen = player->state.amstream_buf_len;
			bitrate = player->media_info.stream_info.bitrate / 8;

			if (bitrate && vbuflen > 0) {
				int64_t ring_cache_ms = (int64_t)(((q->frames_in - q->frames_out) * frame_dur_pts * q->timebase) / 90);
				//ret = (int)abs((vbuflen / bitrate) + (ring_cache_ms - cache_ms ) / 1000);
				log_debug("vbufsize: %lld, vbuflen = %lld, bitrate = %lld, vbufsize / bitrate = %lld ms\n",
							player->state.amstream_buf_size, vbuflen, bitrate, (vbuflen * 1000 / bitrate));
				log_debug("ring_cache_ms = %lld, cache_ms = %lld, ret = %d\n", ring_cache_ms, cache_ms, ret);

				if (!playtime_ms /*|| (ret > 20)*/) {
					total_cache_ms = ring_cache_ms;
					total_cache_ms += (vbuflen * 1000 / bitrate);
					cache_ms = total_cache_ms;
					log_debug("cache_ms: %lld, q->frames_in: %lld, q->frames_out: %lld, q->timebase = %f, playtime_ms = %lld\n",
								cache_ms, q->frames_in, q->frames_out, q->timebase, playtime_ms);
				}
			}
		}
		//end
		...
		*/
		...
		if (player->state.status == PLAYER_BUFFERING) {
			if (cache_ms >= 5000) {
			    log_print("Warnning: cache_ms:%lld, check player status \n", cache_ms);
			    cache_ms = 4500;
			}
		}
		//end
...
int64_t avpkt_cache_update_discontinue_current_ms(play_para_t *player)
{
...

	{
		...
		if (abs(diff_ms) >= CURRENT_TIME_MS_DISCONTINUE) {
			...
			log_print("current time discontinue: %lld - > %lld\n", s_avpkt_cache->last_currenttime_ms, current_ms);
			log_print("discontinue_current_ms: %lld, diff_ms = %lld\n", s_avpkt_cache->discontinue_current_ms, diff_ms);
		}
		s_avpkt_cache->last_currenttime_ms = current_ms;
	}
	return current_ms;
...
int64_t avpkt_cache_getcache_time_by_streamindex(play_para_t *player, int stream_idx)
{
	...
	avpkt_cache_update_discontinue_current_ms(player);
...
int avpkt_cache_set_cmd(AVPacket_Cache_E cmd)
{
	...
	while (retry > 0) {
		if (s_avpkt_cache->state == 0) {
			log_print("%s:%d not inited, return retry:%d\n", __FUNCTION__, __LINE__,retry);
...
int avpkt_cache_search(play_para_t *player, int64_t seekTimeSec)
{
	...
	if (s_avpkt_cache->has_audio == 1 && switch_audio) {
		...
		s_avpkt_cache->queue_audio->seek_bykeyframe = player->playctrl_info.seek_keyframe;
		...
		//s_avpkt_cache->seek_by_keyframe = player->playctrl_info.seek_keyframe;
		s_avpkt_cache->queue_video->seek_bykeyframe = player->playctrl_info.seek_keyframe;
		s_avpkt_cache->seek_by_keyframe_maxnum = am_getconfig_int_def("libplayer.cache.seekkeynum", SEEK_KEY_FRAME_MAX_NUM);
...
static int avpkt_cache_check_netlink(void) {
	...
	*/
	...
	//log_print("debug atoi(acNetStatus):%d, netdown:%d\n", atoi(acNetStatus),s_avpkt_cache->netdown);
	return s_avpkt_cache->netdown;
}
            ...
            if (avpkt_cache_validate_stream_idx(&pkt) == 1) {
                ...
                if (ret == 0){
                    for (; ;) {
                        if (s_avpkt_cache->state != 2) {
                            //state change
                            log_print("[%s %d]s_avpkt_cache->state:%d\n", AVSYNC_TAG,__LINE__, s_avpkt_cache->state);
                            s_avpkt_cache->backup_pkt = pkt;
                            return -1;
                        }
                       ...
                       if (ret == 0) {
                           ...
                           if (s_avpkt_cache->avsync_mode == 1 && s_avpkt_cache->avsyncing == 1) {
...
int avpkt_cache_release_thread_t(av_packet_cache_t *cache_ptr)
{
    ...
    pthread_attr_setstacksize(&pthread_attr, 0);   //default stack size maybe better
    log_print("open %s\n", __FUNCTION__);
    ret = pthread_create(&tid, &pthread_attr, (void*)&avpkt_cache_release_thread, (void*)cache_ptr);
    if (ret != 0) {
        log_print("creat player thread failed !\n");
        return ret;
    }
...
void *cache_worker(void *arg)
{
	...
	while (nRunning == 1) {
		if (avpkt_cache_interrupt(s_avpkt_cache)) {
			...
			log_print("LNXXXXX------->%s:%d reading:%d\n", __FUNCTION__, __LINE__,s_avpkt_cache->reading);
			// log_print("%s:%d reading:%d\n", __FUNCTION__, __LINE__,s_avpkt_cache->reading);
			s_avpkt_cache->reading = 0;
		}
		if (s_avpkt_cache->state == 2) {
			if (player->playctrl_info.pause_cache != 0) {
				amthreadpool_thread_usleep(CACHE_THREAD_SLEEP_US);
				log_print("%s:%d LNXXXXX------->player->playctrl_info.pause_cache != 0,continue\n", __FUNCTION__, __LINE__);
				continue;
			}
			...
			//end
			//update discontinue current_ms
			avpkt_cache_update_discontinue_current_ms(player);

			if (s_avpkt_cache->error == AVERROR_EOF) {
				amthreadpool_thread_usleep(CACHE_THREAD_SLEEP_US);
				log_print("%s:%d LNXXXXX------->s_avpkt_cache->error == AVERROR_EOF,continue\n", __FUNCTION__, __LINE__);
				continue;
			}
			...
			if (avpkt_cache_check_netlink() == 1) {
				//net down, sleep
				amthreadpool_thread_usleep(CACHE_THREAD_SLEEP_US);
				log_print("%s:%d LNXXXXX------->avpkt_cache_check_netlink() == 1,continue\n", __FUNCTION__, __LINE__);
				continue;
			}
			...
			if (s_avpkt_cache->keeframesstate == 2) {
				amthreadpool_thread_usleep(CACHE_THREAD_SLEEP_US);
				log_print("%s:%d LNXXXXX------->s_avpkt_cache->keeframesstate == 2 continue\n", __FUNCTION__, __LINE__);
				continue;
			}
			...
			else if ((am_kpi & 0x400) && s_avpkt_cache->read_frames >=2 && (player) && (player->pFormatCtx) && (player->pFormatCtx->iformat)
				&& (!strncmp(player->pFormatCtx->iformat->name, "mpegts", strlen("mpegts")))
				&& (player->pFormatCtx->nb_streams <= 2) && (player->pFormatCtx->pb)
				&& (player->pFormatCtx->pb->local_playback != 1)){
				amthreadpool_thread_usleep(500);
			}
			log_print("%s:%d LNXXXXX------->s_avpkt_cache->state == 2\n", __FUNCTION__, __LINE__);
			/*end*/
			continue;
		} else if (s_avpkt_cache->state == 0) {
			log_print("%s:%d LNXXXXX------->s_avpkt_cache->state == 0\n", __FUNCTION__, __LINE__);
			break;
		} else if (s_avpkt_cache->state == 1) {
			log_print("%s:%d LNXXXXX------->s_avpkt_cache->state == 1\n", __FUNCTION__, __LINE__);
			last_current_ms = 0;
			current_ms = 0;
			curtime_us = 0;
			starttime_us = 0;
	...
	if (am_kpi & 0x2) {
		...
		log_print("%s:%d LNXXXXX------->avpkt_cache_release_thread_t\n", __FUNCTION__, __LINE__);
	} else {
